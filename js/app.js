...
function renderSiswa(){
    ...
    <div class="section-rekap"><div class="print-only" style="text-align:center;margin-bottom:10px;"><h5>Rekapitulasi Data Siswa KKMA 04 Jember</h5></div><div class="form-section mb-4"><h5 class="mb-3 no-print"><i class="bi bi-table text-primary me-2"></i>Rekapitulasi Per Lembaga</h5>
    ...
    <tfoot><tr class="table-success fw-bold"><td colspan="2" class="text-center">TOTAL KKMA 04</td><td class="text-center">${tot('x_l')||'-'}</td><td class="text-center">${tot('x_p')||'-'}</td><td class="text-center">${tot('xi_l')||'-'}</td><td class="text-center">${tot('xi_p')||'-'}</td><td class="text-center">${tot('xii_l')||'-'}</td><td class="text-center">${tot('xii_p')||'-'}</td><td class="text-center">${_data.siswa.length||'-'}</td></tr></tfoot></table></div></div></div>
    <div class="section-daftar"><div class="print-only" id="printJudulDaftar" style="text-align:center;margin-bottom:10px;"><h5>Daftar Nama Siswa MA KKMA 04 Jember</h5></div><div class="form-section"><div class="d-flex flex-wrap gap-2 mb-3"><select class="form-select form-select-sm" style="width:auto" id="fSMdr"><option value="">Semua Madrasah</option>${_data.madrasah.map(m=>`<option value="${m._id||m.nsm}">${H(m.nama)}</option>`).join('')}</select><select class="form-select form-select-sm" style="width:auto" id="fSKls"><option value="">Semua Kelas</option><option>X</option><option>XI</option><option>XII</option></select><button class="btn btn-outline-secondary btn-sm no-print" id="btnCetakDaftar"><i class="bi bi-printer me-1"></i>Cetak Daftar Nama</button></div>
    ...
    $('btnCetakDaftar')?.addEventListener('click',()=>{
        const sel=fM?.value;
        const mdr=sel?getMadrasah(sel):null;
        const judul=$('printJudulDaftar');
        if(judul){judul.innerHTML='<h5>Daftar Nama Siswa '+(mdr?H(mdr.nama):'MA KKMA 04 Jember')+'</h5>';}
...
function renderDirektori(){
    ...
    <div class="row g-4"><div class="col-md-5">
        ...
        ${Session.isAdmin()?`<button class="btn btn-sm btn-outline-primary" id="btnEditPengawas"><i class="bi bi-pencil me-1"></i>Edit Profil</button> <button class="btn btn-sm btn-outline-success" id="btnFotoPengawas"><i class="bi bi-camera me-1"></i>Upload Foto</button><input type="file" id="inputFoto" accept="image/*" style="display:none">`:''}</div>
...
function renderTentang(){
    ...
    </table>${Session.isAdmin()?`<button class="btn btn-sm btn-outline-primary" id="btnEditProfil"><i class="bi bi-pencil me-1"></i>Edit Profil</button>`:''}</div></div>
    <div class="col-md-6"><div class="form-section h-100"><h5>Visi & Misi</h5><p class="fw-bold small">${H(_profil.visi)}</p><h6 class="small">Misi:</h6><ol class="small">${(_profil.misi||[]).map(m=>`<li>${H(m)}</li>`).join('')}</ol>${Session.isAdmin()?`<button class="btn btn-sm btn-outline-primary" id="btnEditVisiMisi"><i class="bi bi-pencil me-1"></i>Edit Visi & Misi</button>`:''}</div></div>
    ...
    ${Session.isAdmin()?`<button class="btn btn-sm btn-outline-primary" id="btnEditSambutan"><i class="bi bi-pencil me-1"></i>Edit Sambutan</button> <button class="btn btn-sm btn-outline-success" id="btnFotoTentang"><i class="bi bi-camera me-1"></i>Upload Foto</button><input type="file" id="inputFotoTentang" accept="image/*" style="display:none">`:''}</div></div></div>`;
    // Edit Profil
